 ...
 */
const startInstance = async (jobId, hostdomain, callbackArray) => {
  console.log('')
  ...
  page.on('request', async (request) => {
    if (request.isNavigationRequest() && request.redirectChain().length) {
      ...
      await page.screenshot({ path: `screenshots\\redirected-${hostdomain}-${jobId}.png` })
  ...
  try {
    if (hostdomain === 'indeed') {
      let __return
      ({ __return, redirected, status } = await crawlIndeed(jobId, page, data, redirected, status, callbackArray))
      return __return
      ...
      let __return
      ({ __return, status } = await crawlLinkedIn(jobId, page, data, redirected, status, callbackArray))
      return __return
    }
  } catch (error) {
...
async function crawlLinkedIn(jobId, page, data, redirected, status, callbackArray) {
  ...
  if (!redirected && response.status() === 200) {
    ...
    try {
      ...
      await page.screenshot({ path: `screenshots\\positionStatus-linkedin-${jobId}.png` })
      data.crawlStatus.push(JSON.stringify(error))

    }

    if (data.status === 'open' && callbackArray?.length >= 1) {
      for (const callback of callbackArray) {
        await callback(page, data, jobId)
      }
    }
...
async function crawlIndeed(jobId, page, data, redirected, status, callbackArray) {
  ...
  if (pageTitle.includes('Page Not Found')) {
    await page.screenshot({ path: `screenshots\\404-indeed-${jobId}.png` })
    redirected = true
    status = 'closed'
  }
  ...
  if (!redirected && response.status() === 200) {
    ...
    try {
      ...
      console.log(chalk.red('Could not determine position status'))
      await page.screenshot({ path: `screenshots\\positionStatus-indeed-${jobId}.png` })
      data.crawlStatus.push(JSON.stringify(error))
    }
